...
//              digest = EasyUtils.getAppResourceString(context, "location_prefix");
     ...
     * @param user
     */
    public static void setUserInitialLetter(ContactInfo user) {
    ...
    public static void setMessageAtMember(EMMessage message, String... atMembers) {
        List<String> atMembersList = new ArrayList<>();
        for (String atMember : atMembers) {
            if (atMember.equals(EaseConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL)) {
                // @all 时特殊处理
                message.setAttribute(EaseConstant.MESSAGE_ATTR_AT_MSG, EaseConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL);
                return;
            }
            atMembersList.add(atMember);
        }
        JSONArray atJson = new JSONArray(atMembersList);
        // 设置消息的扩展为@群成员类型
        message.setAttribute(EaseConstant.MESSAGE_ATTR_AT_MSG, atJson);
    }
    
    public static ArrayList<String> getMessageAtMembers(EMMessage message) {
        ArrayList<String> memberList = new ArrayList<>();
        try {
            String atJson = message.getStringAttribute(EaseConstant.MESSAGE_ATTR_AT_MSG);
            if (EaseConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL.equals(atJson)) {
                memberList.add(EaseConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL);
            }
            else {
                JSONArray atList = message
                        .getJSONArrayAttribute(EaseConstant.MESSAGE_ATTR_AT_MSG);
                for (int i = 0; i < atList.length(); i++) {
                    try {
                        memberList.add((String) atList.get(i));
